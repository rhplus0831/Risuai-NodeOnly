...
| `server/node/db/maintenanceRoutes.cjs` | Storage dashboard and database maintenance routes: `/api/db/stats` (+`/characters`, `/modules`), `/api/db/optimize`, `/api/db/durability` GET/PUT, `/api/db/wal-checkpoint`, non-restore `/api/db/snapshots` (limits/list/delete), and `/api/assets/cleanup`. The durability/WAL engine, snapshot-limit persistence and rotation, and the destructive `/api/db/snapshots/restore` route stay in `server.cjs`; mutations cross only through named server-side wrappers such as `persistSqliteDurabilityMode()`, and mutable paths/ETag are read through getters at request time. |
