...
function startProxyStreamJobGc() {
    setInterval(() => {
        const now = Date.now();
...
const reverseProxyFunc = async (req, res, next) => {
    ...
    if(req.headers['authorization']?.startsWith('X-SERVER-REGISTER')){
        ...
        const proxyTarget = assertProxyTargetAllowed(urlParam, {
            enforceInternalBlock: isHubHostingMode(),
        });
...
function registerProxyRoutes(app, ctx) {
    const {
        checkAuth: registeredCheckAuth,
        logger: registeredLogger,
        isHubHostingMode: registeredIsHubHostingMode,
    } = ctx;
    checkAuth = registeredCheckAuth;
    logger = registeredLogger;
    isHubHostingMode = registeredIsHubHostingMode;
